...
/**
 * Consolidates the project.project_details.js_files.
 * This should extract the file names and contents out, and only leave
 * the names in the consolidated data.rpf file. There should be
 * separate JS files created along with data.rpf.
 * @param {!Object} details The project details.
 * @return {Object} The modified data file and the JS files string.
 * @private
 */
rpf.DataModel.prototype.consolidateProjectDetails_ = function(details) {
  if (!details['js_files'] || details['js_files'].length == 0) {
    return {'details': details, 'jsFiles': ''};
  }
  var newDetails = goog.object.unsafeClone(details);
  var jsFiles = [];
  var files = newDetails['js_files'];
  for (var i = 0; i < files.length; ++i) {
    jsFiles.push({'name': files[i]['name'],
                  'code': files[i]['code']});
    delete files[i]['code'];
  }
  return {'details': newDetails, 'jsFiles': goog.json.serialize(jsFiles)};
};
...
rpf.DataModel.prototype.consolidateData = function(projectInfo) {
  var newDetails = this.consolidateProjectDetails_(
      projectInfo['project_details']);
  var jsFiles = newDetails['jsFiles'];
  var result = {'name': projectInfo['name'],
                'project_details': newDetails['details'],
                'tests': []};
  ...
  return {'data': result, 'jsFiles': jsFiles};
};
