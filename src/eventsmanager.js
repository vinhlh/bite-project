...
rpf.EventsManager = function() {
  ...
  /**
   * The function to get the common libs string.
   * @type {Function}
   */
  this.getCommonLibsAsStr = null;
 ...
 * @param {function():string} getCommonLibsAsStr Gets the common libs string.
...
rpf.EventsManager.prototype.setupCommonFuncs = function(
    getCommonLibsAsStr, getCommonLibs, setCommonLib, rpfListener) {
  /**
   * The function to get the common libs string.
   */
  this.getCommonLibsAsStr = getCommonLibsAsStr;
 ...
 * @export
 */
rpf.EventsManager.prototype.executeScriptStr = function(id, script) {
  chrome.tabs.executeScript(
      id,
      {code: script, allFrames: true}
  );
};
...
rpf.EventsManager.prototype.callBackOnMessageReceived = function(
  ...
  switch (request['command']) {
    ...
    case Bite.Constants.CONSOLE_CMDS.SAVE_JSON_LOCALLY:
      if (params['override']) {
        params['userLib'] =
            this.getCommonLibs_()[Bite.Constants.TEST_LIB_NAME]['lib'];
      }
    ...
    case Bite.Constants.CONSOLE_CMDS.CHECK_PLAYBACK_OPTION_AND_RUN:
      ...
      params['userLib'] =
          this.getCommonLibsAsStr([Bite.Constants.TEST_LIB_NAME]) +
          '\n' + params['userLib'];
...
rpf.EventsManager.prototype.callBackAddTestTabRemoved = function(tabId) {
  if (this.recordMgr_.getTestTabId() == tabId) {
    var log = ['Chrome reported the tab under record was closed.',
               'Please reset the tab under record.'].join('\n');
    this.stopRecordingFromUi();
    this.sendMessageToConsole_(
        {'command': Bite.Constants.UiCmds.SET_CONSOLE_STATUS,
         'params': {'message': log,
                    'color': 'red'}});
  }
};
