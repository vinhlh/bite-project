...
/**
 * @fileoverview Unit tests for the overview tab in explore page.
 *
...
function testHandleUserOperation() {
  var event = {'target': {'id': 'start'}};
  var view = new bite.server.explore.SetTab();
  view.handleUserOperation(event);
  var sendInstance = goog.testing.net.XhrIo.getSendInstances()[0];
  var sentUri = sendInstance.getLastUri();
  assertEquals('/run/add', sentUri);
}
