...
rpf.CodeGenerator.testCmdType = function(cmd) {
  ...
  if (goog.string.startsWith(cmd, actions.RUN)) {
    return 0;
      ...
      goog.string.startsWith(cmd, actions.CALL) ||
...
rpf.CodeGenerator.getUrlInRedirectCmd = function(cmd) {
  // Support either the new version of the redirect command,
  // redirectTo("url"), and the old redirectTo(url).
  var urlRegexp = /\("?([^"]*)"?\)/;
  return urlRegexp.exec(cmd)[1];
};
...
rpf.CodeGenerator.prototype.translateCmd = function(
  ...
  } else if (script.indexOf(rpf.CodeGenerator.PlaybackActions.CALL) == 0) {
    humanReadable = 'Runs a custom JS function.';
  } else {
...
rpf.CodeGenerator.getCmdUniqueId = function(
  ...
  try {
    ...
    var cmdClass = getText(descObj['attributes']['class']);
    var cmdId = getText(descObj['attributes']['id']);
    var temp = text ? text : value ? value : cmdId ? cmdId : cmdClass;
    if (temp) {
